...
function mkLogEmitter(minLevel) {
  return function(...args) {
    ...
    const msgArgs = new Array(args.length);
...
if (mv) {
  ...
  RotatingFileStream = function RotatingFileStream(options) {
    ...
    assert.equal(typeof this.count, 'number', `rotating-file stream "count" is not a number: ${this.count} (${typeof this.count}) in ${this}`);
    assert.ok(this.count >= 0, `rotating-file stream "count" is not >= 0: ${this.count} in ${this}`);
